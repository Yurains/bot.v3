...
try:
    ...
    pass
#----------------------------------------------------
CONST_APP_VERSION = "TicketRobot v3."
...
CONST_CAPTCHA_SOUND_FILENAME_DEFAULT = "./img/ding-dong.wav"
...
def load_translate():
    ...
    zh_tw["ocr_captcha_ddddocr_beta"] = 'ddddocr Final'
    ...
    zh_tw["config_launcher"] = '設定檔管理(只有win能用哦)'
    zh_tw["done"] = '完成'
    zh_tw["tixcraft_sid"] = '拓元cookie SID(現已失效)'
...
def PreferenctTab(root, config_dict, language_code, UI_PADDING_X):
    ...
    group_row_count += 1
#----------------------------------------------------------------------------------------
    cat_img = PhotoImage(file='./img/cat.ppm')
    cat_img_small = cat_img.zoom(9, 9).subsample(10, 10)
    lbl_cat = Label(frame_group_area, image=cat_img_small)
    lbl_cat.image = cat_img_small  # 防止圖片被垃圾回收
    lbl_cat.grid(column=1, row=group_row_count, sticky=W)
    group_row_count += 1
...
def AdvancedTab(root, config_dict, language_code, UI_PADDING_X):
    ...
    if len(play_sound_filename)==0:
        ...
        play_sound_filename_default = "./img/ding-dong.wav"
...
#----------------------------------------------------------------------------------------
    ...
    icon_play_filename = "./img/icon_play_1.gif"
...
def VerificationTab(root, config_dict, language_code, UI_PADDING_X):
    ...
    icon_preview_text_filename = "./img/icon_chrome_4.gif"
...
def ServerTab(root, config_dict, language_code, UI_PADDING_X):
    ...
    icon_copy_filename = "./img/icon_copy_2.gif"
    ...
    icon_query_filename = "./img/icon_query_5.gif"
...
def settgins_gui_timer():
    while True:
        ...
        if GLOBAL_SERVER_SHUTDOWN:
            break
def btn_paste_answer_by_user(event=None):
    try:
        clipboard_text = pyperclip.paste()
        txt_answer.delete(0, END)
        txt_answer.insert(0, clipboard_text)
    except Exception as e:
...
def AboutTab(root, language_code):
    ...
    frame_group_header = Frame(root)
    group_row_count = 0
    frame_group_header.grid_columnconfigure(0, weight=1)
    # 顯示 LOGO
    logo_filename = "./img/images.ppm"
    logo_img = PhotoImage(file=logo_filename)
    lbl_logo = Label(frame_group_header, image=logo_img)
    lbl_logo.image = logo_img
    lbl_logo.grid(column=0, row=group_row_count, columnspan=2, sticky="nsew")
    group_row_count += 1
    lbl_contact = Label(frame_group_header,
                        text="聯絡作者：https://www.instagram.com/junyan_0826",
                        fg="blue", cursor="hand2", justify="center")
    lbl_contact.grid(column=0, row=group_row_count, columnspan=2, pady=10, sticky="nsew")
    lbl_contact.bind("<Button-1>", lambda e: webbrowser.open("https://www.instagram.com/junyan_0826"))
    group_row_count += 1
    root.grid_columnconfigure(0, weight=1)
    frame_group_header.grid(column=0, row=0, padx=15, pady=15)
    global lbl_slogan
    global lbl_help
    global lbl_donate
    global lbl_release
