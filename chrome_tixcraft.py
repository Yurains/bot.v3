...
def tixcraft_area_auto_select(driver, url, config_dict):
    print("run tixcraft_area_auto_select")
    show_debug_message = True       # debug.
    # show_debug_message = False      # online
    ...
    if not el is None:
        ...
        try:
            ...
            driver.refresh()
            # 等待頁面加載，根據 config 或預設 5 秒
            wait_time = config_dict["advanced"].get("auto_reload_page_interval", 5)
            ...
            for area_keyword_item in area_keyword_array:
                # 使用更穩健的 XPath 搜尋
                matched_blocks = driver.find_elements(By.XPATH, f"//*[contains(., '{area_keyword_item}')]")
                if not matched_blocks:
                    # 如果還是找不到，嘗試更寬鬆的條件
                    matched_blocks = driver.find_elements(By.XPATH, f"//p[contains(text(), '{area_keyword_item}')]")
                if matched_blocks:
                    break
                else:
                    print("is_need_refresh for keyword:", area_keyword_item)
            ...
            if matched_blocks:
                if show_debug_message:
                    print(f"Found {len(matched_blocks)} elements matching '{area_keyword_item}'")
                # 根據模式選擇目標
                target_area = util.get_target_item_from_matched_list(matched_blocks, auto_select_mode)
                if target_area:
                    try:
                        print("Target text:", target_area.text)
                        target_area.click()
                        print(f"Clicked {area_keyword_item} successfully!")
                        return True
                    except Exception as exc:
                        print("Click failed, retrying with JS...")
                        driver.execute_script("arguments[0].click();", target_area)
                        return True
                else:
                    print("No target selected from matched blocks.")
            else:
                print(f"No elements found with '{area_keyword_item}' after refresh.")
                # 輸出頁面內容以便檢查
...
def tixcraft_main(driver, url, config_dict, ocr, Captcha_Browser):
    ...
    # choose area
    if '/ticket/area/' in url or '/events/' in url:
        domain_name = url.split('/')[2]
        print("1111",domain_name)
        if config_dict["area_auto_select"]["enable"]:
            if not 'ticketmaster' in domain_name or "kktix.cc" in domain_name:
                # for tixcraft
